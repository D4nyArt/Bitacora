 ...
 * Complejidad O(n^2)
 *
 * @param consultaRes es un vector de eventos que regresada por la funcion
 * consulta
 */
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            if (bitacoraPtr == nullptr) {
                cout << "La bitacora no ha sido creada todav�a" << endl;
            } else {
                ...
                cin.ignore();  // Limpiar el buffer del teclado antes de usar
                               // getline
        ...
        if (choice == 4) {
            if (bitacoraPtr == nullptr) {
                ...
                cout << "Ingrese el nombre del archivo donde se guardara la "
                        "bitacora ordenada: ";
        ...
        if (choice == 6) {
            ...
            cout << "Esa opcion no se encuentra en el menu, intenta de nuevo"
                 << endl;
        }
    }
    delete bitacoraPtr;
