	...
	do{
		...
		case 2:
			if(!bitacora_creada){
				...
				cout<<"Indica el mes (coloca las primeras 3 letras): "<<endl;
				cin>>registro[0];
				while(registro[0].size()!=3){
			        cout<<"El mes debe tener exactamente 3 letras. Ingrese nuevamente: ";
			        cin.ignore();
			        cin>>registro[0];
			    }
				...
				while (registro[2].size()!=8 || registro[2][2]!=':' || registro[2][5]!=':' || stoi(registro[2].substr(0, 2))>23 || stoi(registro[2].substr(3, 2))>59 || stoi(registro[2].substr(6, 2))>59){
			        cout<<"Asegúrese de cumplir con el formato de hora e ingrese nuevamente: ";
			        cin.ignore();
			        cin>>registro[2];
			    }
				cout<<"Indica la direccion IP (en formato xxx.xx.xxx.xx:xxxx): "<<endl;
				cin>>registro[3];
		...
		case 3:
			if(!bitacora_creada){
				...
				cout<<"Ingrese el lote completo (Mes Dia Hora IP Falla): "<<endl;
				cin.ignore();
				getline(cin, lote);
				B.CargaLotes(lote);
